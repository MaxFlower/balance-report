...
const BalanceSheetRow: React.FC<IBalanceSheetRowProps> = ({ row, id }) => {
    const buildCells = (cells: ReportCell[] = []) => {
        return (
            <Table.Tr>
                {cells?.map((c, i) => <Table.Td key={`${ROW_TYPE.SummaryRow}-${id}-${i}`}>{c.Value}</Table.Td>)}
            </Table.Tr>
        )
    };
    ...
    const highlightedRow = (title: string, children: React.ReactNode | null) => {
        return (
            <>
                <Table.Thead>
                    <Table.Tr>
                    <Table.Th><Text c="blue">{title}</Text></Table.Th>
                    </Table.Tr>
                </Table.Thead>
    ...
    const buildSection = () => {
        return (
            <>
                ...
                {row.Rows && row.Rows.map((secRow, rId) => <BalanceSheetRow row={secRow} id={rId} />)}
            </>
        )
    }
  ...
  const elements: Record<ROW_TYPE, () => React.ReactNode> = {
    ...
    [ROW_TYPE.SummaryRow]: () => highlightedRow('Summary', buildCells(row.Cells)),
  };
