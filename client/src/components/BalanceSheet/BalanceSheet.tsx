...
//   ));
  ...
  if (loading) return (
    <><Loader color="blue" /><p>Loading</p></>
  );
  if (error || !report) return (
    <Alert variant="light" color="red" title="Error occured" icon={<IconInfoCircle />}>
      {error}
    </Alert>
  );
  ...
  return (
    <>
        <Text fw={700}>{report?.ReportID}: {report.ReportTitles.join(' ')}</Text>
        <Table>
            {report.Rows.map((r, idx)=> <BalanceSheetRow row={r} id={idx} key={`BalanceSheetRow-${idx}`} />)}
        </Table>
    </>
  );
};
